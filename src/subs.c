...
int mqtt3_db_messages_queue(struct mosquitto_db *db, const char *source_id, const char *topic, int qos, int retain, struct mosquitto_msg_store **stored)
{
    ...
    */
    ...
    if (tokens != NULL && !strcmp("$USR", tokens->topic) && tokens->next != NULL && tokens->next->topic != NULL)
    {
        uint16_t mid;
        struct mosquitto *context;
        int len;

        HASH_FIND(hh_id, db->contexts_by_id, tokens->next->topic, strlen(tokens->next->topic), context);
        ...
        if (context)
        {
            ...
            len = strlen("$USR/") + strlen(source_id) + 1;
            (*stored)->topic = _mosquitto_realloc((*stored)->topic, len);
            if ((*stored)->topic == NULL)
            {
                return MOSQ_ERR_NOMEM;
            }
            snprintf((*stored)->topic, len, "$USR/%s", source_id);
            
            _mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "%s", (*stored)->topic);

            if (mqtt3_db_message_insert(db, context, mid, mosq_md_out, qos, false, *stored)) rc = 1;
        }
    }
    else
    {
